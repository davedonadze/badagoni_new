...
`/admin` is a password-protected panel (see `app/admin/`) for managing wines (`/admin/wines`, including bottle images uploaded to R2), wine categories (`/admin/categories`), the header/footer navigation (`/admin/menu`), and every page's text/images (`/admin/pages` — home, story, contact, terroir, the Alaverdi Monastery Cellar page, and enologists each have a dedicated editor for their own fields; a page's layout and animations stay in code, only its fields are editable; "+ Add page" creates new pages on a simpler generic template) without a code deploy. Every bilingual field has an English/Georgian pair and a "Translate" button backed by the Claude API; the public site currently renders the English side only. Full setup instructions — local dev, provisioning the real D1 database and R2 bucket, and setting `ADMIN_PASSWORD`/`ANTHROPIC_API_KEY` — are in `docs/admin-panel.md`.
